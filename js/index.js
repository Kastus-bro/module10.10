  ...
  for (let i = 0; i < fruits.length; i++) {
    ...
    switch (fruits[i].kind) {
      ...
      case 'Тамаринд':
        ...
        break
      default:
        liFruit.className = 'fruit__item fruit_blue';
    }

    let divFruit = document.createElement('div'); // div 
    divFruit.className = 'fruit__info'; // класс
    liFruit.appendChild(divFruit); // создаем список li

    //создаем div для фрукта:
    ...
    fruitsList.appendChild(liFruit); // добавляем список li в ul fruitsList
  }
};
...
function shuffleFruits() {
  ...
  //перемешиваем еще раз если результат совпадает:
  if (JSON.stringify(fruits) === JSON.stringify(fruitsForShuffle)) {
    alert('Перемешайте еще раз');
  }
  // на практике результат совпадения маловероятен
...
function filterFruits() {
  ...
  if (parseInt(min) <= 0 || isNaN(parseInt(min)) || parseInt(max.value) <= 0 || isNaN(parseInt(max)) || min > max) {
    alert('Неправильные значения, введите еще раз!');
  }
  //записываем условия фильтрации
  else {
...
addActionButton.addEventListener('click', () => {
  ...
  if (kindInput.value === '' || colorInput.value === '' || weightInput.value === '' || parseInt(weightInput.value) <= 0 || isNaN(parseInt(weightInput.value))) {
    ...
    // добавляем его к массиву и выводим на экран:
